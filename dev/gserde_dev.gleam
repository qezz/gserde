import gleam/list
import gleam/string
import gleeunit/should
import shellout
import simplifile

const foo_module = "
import gleam/option.{type Option, Some}

pub type FooJson {
  Foo(
    a_bool: Bool,
    b_int: Int,
    c_float: Float,
    e_option_int: Option(Int),
    f_string_list: List(String),
  )
}

pub fn fixture_foo() {
  Foo(
    a_bool: True,
    b_int: 1,
    c_float: 1.0,
    e_option_int: Some(4),
    f_string_list: [\"a\", \"b\"]
  )
}
"

const bar_module = "
import internal/tmp_out/foo

pub type BarJson {
  Bar(
    foo: foo.FooJson
  )
}
"

const foo_json_test = "
import gleam/io
import gleam/result
import internal/tmp_out/foo
import internal/tmp_out/foo_json

pub fn main() {
  let foo_a = foo.fixture_foo()

  let foo_str = foo_a
    |> foo_json.to_string

  let foo_b = foo_str |> foo_json.from_string |> result.lazy_unwrap(fn() {
    echo \"parse error calling foo_json.from_string\"
    panic
  })

  case foo_a == foo_b {
    True -> io.println(\"foos equal\")
    False -> {
      echo #(\"a\", foo_a)
      echo #(\"b\", foo_b)
      panic as \"not equal\"
    }
  }

  io.print(foo_str)
}
"

const bar_json_test = "
import gleam/io
import gleam/result
import internal/tmp_out/foo
import internal/tmp_out/bar
import internal/tmp_out/bar_json

pub fn main() {
  let bar_a = bar.Bar(
    foo: foo.fixture_foo()
  )

  let bar_str = bar_a
    |> bar_json.to_string

  let bar_b = bar_str |> bar_json.from_string |> result.lazy_unwrap(fn() {
    echo \"parse error calling bar_json.from_string\"
    panic
  })

  case bar_a == bar_b {
    True -> io.println(\"bars equal\")
    False -> {
      echo #(\"a\", bar_a)
      echo #(\"b\", bar_b)
      panic as \"not equal\"
    }
  }

  io.print(bar_str)
}
"

fn exec(bin: String, args: List(String)) {
  let assert Ok(output) =
    shellout.command(bin, args, in: ".", opt: [shellout.LetBeStderr])
  output
}

//
/// This test is thorough, but a bit zany. We:
/// - write a dummy "foo" module into the current project's source. hopefully it compiles!
/// - run our CLI against the project, which should produce a serde module next to foo
/// - write an entrypoint module--foo_json_test--into our source, then run it!
pub fn main() {
  let _ = simplifile.create_directory("tmp_out")
  let assert Ok(_) =
    simplifile.write(to: "tmp_out/foo.gleam", contents: foo_module)

  let assert Ok(_) =
    simplifile.write(to: "tmp_out/bar.gleam", contents: bar_module)

  let assert Ok(_) = simplifile.rename("tmp_out", "src/internal/tmp_out")

  // run our gen cli
  exec("gleam", ["run", "--", "src/internal/tmp_out"])

  // write our test module and run it
  let assert Ok(_) =
    simplifile.write(
      to: "src/internal/tmp_out/foo_json_test.gleam",
      contents: foo_json_test,
    )

  let assert Ok(_) =
    simplifile.write(
      to: "src/internal/tmp_out/bar_json_test.gleam",
      contents: bar_json_test,
    )
  // let assert Ok(_) = simplifile.rename("tmp_out", "src/internal/tmp_out")

  let assert Ok(last_foooutput_line) =
    exec("gleam", ["run", "-m=internal/tmp_out/foo_json_test"])
    |> string.split("\n")
    |> list.last

  last_foooutput_line
  |> should.equal(
    "{\"a_bool\":true,\"b_int\":1,\"c_float\":1.0,\"e_option_int\":4,\"f_string_list\":[\"a\",\"b\"]}",
  )
}
